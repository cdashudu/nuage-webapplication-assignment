...
app = Flask(__name__)
app.secret_key = 'nuage network'
mysql = MySQL()
 
# MySQL configurations
app.config['MYSQL_DATABASE_PASSWORD'] = 'Root!234'
app.config['MYSQL_DATABASE_PASSWORD'] = 'Root!23'
app.config['MYSQL_DATABASE_DB'] = 'BucketList'
app.config['MYSQL_DATABASE_HOST'] = '10.0.0.241'
...
def deleteWish():
    try:
        if session.get('user'):
            _id = request.form['id']
            _user = session.get('user')
            print("This was called definetly")
            conn = mysql.connect()
            cursor = conn.cursor()
